...
function fetch(event) {
  return Promise.map(event, item => {
    ...
    return browser
      .load(url)
      .then(
        ({ links, html, screenshotData }) =>
          storeItems(seed, url, html, screenshotData).then(() =>
...
function sendResult(seed, url, result) {
  return frontierUrlPromise.then(frontierUrl =>
    signedAxios({
      method: 'PATCH',
      ...
      .catch(err => {
        log.error({ data: err.response.data }, 'Error Data')
        throw err
      })
      .then(() => result)
  )
...
function storeItems(seed, url, html, screenshotData) {
  const keyPrefix = `${encodeURIComponent(seed)}/${encodeURIComponent(url)}`
